...
export function tick(
  ...
  while (
    ...
    state.frameTimes[0] <= performance.now() - fps_measure_window
  ) {
  ...
  ctx.fillStyle = "white";
  ctx.fillText(
    `button clicked ${state.count} ${plural("time", state.count)}`,
  ...
  if (state.name.value !== "") {
    ctx.fillText(
      `Hello, ${state.name.value}!`,
      x + textboxWidth + margin,
      y + textboxHeight / 2,
    );
  }
